//this file is generated by codedump at 2/12/2019 15:07:58 do NOT edit it !

#include "anniversary_campaign_parser.h"
...
{
    if(from_str_variant(root->get_child("type"), info.type)<0) {return false;}
    if(from_str_variant(root->get_child("boy_id"), info.boy_id)<0) {return false;}
    if(from_str_variant(root->get_child("girl_id"), info.girl_id)<0) {return false;}
    if(from_str_variant(root->get_child("num"), info.num)<0) {return false;}
...
{
    {
        variant* v_class = root->get_child("need");
        if(v_class &&
        !ParseRewardInfo(v_class, info.need))
        {
            return false;
        }
    }
...
{
    if(from_str_variant(root->get_child("campaign_id"), info.campaign_id)<0) {return false;}
    if(from_str_variant(root->get_child("carnival_activity"), info.carnival_activity)<0) {return false;}
...
{
    if(from_str_variant(root->get_child("seat_a_id"), info.seat_a_id)<0) {return false;}
    if(from_str_variant(root->get_child("seat_b_id"), info.seat_b_id)<0) {return false;}
    if(from_str_variant(root->get_child("seat_c_id"), info.seat_c_id)<0) {return false;}
...
{
    if(from_str_variant(root->get_child("seat_id"), info.seat_id)<0) {return false;}
    if(from_str_variant(root->get_child("next_seat_id"), info.next_seat_id)<0) {return false;}
    {
        variant* v_class = root->get_child("need");
        if(v_class &&
...
{
    {
        variant* v_class = root->get_child("Misc");
        if(v_class &&
    ...
    {
        variant* v_list = root->get_child("Upgrades");
        if(v_list == NULL)
        {
            v_list = v;
        }
...
{
    if(from_str_variant(root->get_child("dayid"), info.dayid)<0) {return false;}
    {
        variant* v_class = root->get_child("reward");
        if(v_class &&
        !ParseRewardInfo(v_class, info.reward))
        {
            return false;
        }
    }
...
bool AnniversaryCampaignParser::ParseAnniversaryConfig(variant* root, AnniversaryConfig& info)
{
    ...
    {
        variant* v_class = root->get_child("seat");
        if(v_class &&
