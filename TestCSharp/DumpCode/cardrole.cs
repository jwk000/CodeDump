...
namespace GYXMLData
{
    ...
    public class score :IXmlParser
    {
        ...
        public bool Parse(XmlNode node)
        {
            ...
            if(node.Attributes!=null && node.Attributes["longscore"]!=null)
            {
                longscore = int.Parse(node.Attributes["longscore"].Value);
            }
            else
            {
                var child = node.SelectSingleNode("longscore");
                if(child!=null)
                {
                    longscore = int.Parse(child.InnerText);
                }
            }
    ...
    public class Testcardrole : ITestCode
    {
        public void RunTestCode(string basepath)
        {
            string xml = basepath+"test/testxml/cardrole.xml";
            XmlDocument doc = new XmlDocument();
            doc.Load(xml);
            XmlNode root = doc.LastChild;
            cardroleData data = new cardroleData();
            if(!data.Parse(root))
            {
            ...
            FileStream fs = File.Create("cardrole.bytes");
            fs.Write(b, 0, b.Length);
            fs.Close();

            //dump内容
            cardroleData d = GenericParser.GenericsDecode<cardroleData>(b);
            string s = GenericDump.Dump(d);
            fs = File.Create("cardrole.txt");
